...
from webdriver_installer.constants import GECKO_VERSION_URL, GECKO_DOWNLOAD_URL, GECKO_INSTALL_DIR
from webdriver_installer.default_driver import DefaultDriver


class FirefoxDriver(DefaultDriver):
    def __init__(self, bit=32):
        super().__init__(name="GeckoDriver",
                         file_name="geckodriver",
                         bit=bit,
                         version_url=GECKO_VERSION_URL,
                         download_url=GECKO_DOWNLOAD_URL,
                         install_path=GECKO_INSTALL_DIR)
    ...
    def _downloadDriver(self, version) -> bool:
        URL = f"{self._download_url}/download/v{version}/geckodriver-v{version}-win{self._bit}.zip"
