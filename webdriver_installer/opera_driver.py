...
from webdriver_installer.constants import OPERA_VERSION_URL, OPERA_DOWNLOAD_URL, OPERA_INSTALL_DIR
from webdriver_installer.default_driver import DefaultDriver


class OperaDriver(DefaultDriver):
    def __init__(self, bit=32):
        super().__init__(name="OperaDriver",
                         file_name="operadriver",
                         bit=bit,
                         version_url=OPERA_VERSION_URL,
                         download_url=OPERA_DOWNLOAD_URL,
                         install_path=OPERA_INSTALL_DIR)
    ...
    def _getLatestVersion(self) -> str:
        try:
            ...
            version = response.url.split("/")[-1][2:]
            return version
        except:
    ...
    def _downloadDriver(self, version) -> bool:
        ...
        URL = f"{self._download_url}/v.{version}/operadriver_win{self._bit}.zip"
